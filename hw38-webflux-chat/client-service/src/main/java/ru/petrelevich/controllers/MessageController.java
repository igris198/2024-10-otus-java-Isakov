...
public class MessageController {
    ...
    public void getMessage(@DestinationVariable("roomId") String roomId, Message message) {
        ...
        if (String.valueOf(MagicController.MAGIC_ROOM).equals(roomId)) {
            logger.info("Don’t try to write in the magic room!");
            return;
        }

        saveMessage(roomId, message).subscribe(msgId -> logger.info("message send id:{}", msgId));
        convertAndSendMessages(roomId, new Message(HtmlUtils.htmlEscape(message.messageStr())));
    }
    ...
    private Flux<Message> getMessagesByRoomId(long roomId) {
        return datastoreClient
                .get()
                .uri(roomId == MagicController.MAGIC_ROOM ? "/allMsg" : String.format("/msg/%s", roomId))
    ...
    private void convertAndSendMessages(String roomId, Object payload) {
        template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, roomId), payload);
        template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, MagicController.MAGIC_ROOM), payload);
    }
}
